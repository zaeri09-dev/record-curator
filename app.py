        ...
        with st.spinner(f"'{student_name_input}' 학생의 보고서를 LangChain AI가 분석 중입니다... 🤖"):
            try:
                ...
                prompt_template = PromptTemplate(
                    template="""
                    [System Role]
                    너는 통찰력 있고 매우 엄격한 고등학교 과학 교사이자 평가 위원이야. 
                    아래 학생이 제출한 탐구 보고서를 읽고, 무조건적인 칭찬을 배제한 채 극도로 냉정하고 객관적으로 평가해 줘. 잘하는 학생과 부족한 학생의 점수 격차가 확실히 드러나도록 기본 점수 남발을 엄격히 금지해.

                    [Evaluation Criteria]
                    아래 기준을 꼼꼼하게 적용하여 1~100점 사이의 점수를 부여해.

                    1. 과학적탐구력 (int, 1~100)
                    - 90~100점: 조작/종속/통제 변인이 완벽하며, 가설과 실험 설계가 과학적으로 타당함.
                    - 70~89점: 기본 변인 통제는 수행했으나 실험 설계가 다소 평이하거나 세부 조건이 누락됨.
                    - 69점 이하: 과학적 가설에 논리적 오류가 있거나 변인 통제에 완전히 실패함. (부족하다면 과감히 50점 이하 부여)

                    2. 문제해결력 (int, 1~100)
                    - 90~100점: 실험 중 발생한 오차나 예외 결과를 깊이 분석하고, 과학적인 후속 대안을 스스로 도출함.
                    - 70~89점: 오차 원인을 인지하였으나 해결책이 표면적이거나 교과서적임.
                    - 69점 이하: 오차에 대한 고찰이 아예 없거나 예상치 못한 결과를 무시함.

                    3. 논리적사고력 (int, 1~100)
                    - 90~100점: 수집된 데이터와 실험 결과에만 엄격하게 근거하여 결론을 도출함. 논리적 비약 없음.
                    - 70~89점: 결론은 존재하나, 데이터 분석 과정에 약간의 논리적 비약이나 확대 해석이 포함됨.
                    - 69점 이하: 데이터와 무관한 주관적 결론을 내리거나 비논리적임.

                    [과학적 오개념 평가 및 감점 규칙]
                    학생의 보고서에서 과학적 오개념이나 치명적 오류가 발견될 경우, 위 3가지 역량 점수에서 각각 최소 10점 이상 대폭 감점해. 그리고 발견된 오개념과 이를 어떻게 교정해야 하는지를 반드시 아래 '세특초안'에 명시해.
                    ...
                    세특초안 작성 세부 규칙: 
                    - 300자 이내로 간결하게 작성할 것.
                    - 과장된 칭찬이나 미사여구를 빼고, 학생의 실제 강점과 약점(또는 오개념 교정 내용)을 구체적 실험 내용을 바탕으로 담백하게 작성할 것.
                    - 반드시 '~함.', '~보임.', '~분석함.' 등의 명사형 종결어미로 끝낼 것.
                    ...
                    with st.spinner("AI가 기록을 매끄럽게 다듬고 연결하고 있습니다... ✍️"):
                        try:
                            ...
                            summary_prompt = PromptTemplate(
                                template="""
                                [System Role]
                                당신은 통찰력 있는 고등학교 과학 교사이자, 대학 입학 사정관의 시선을 가진 '생기부 기록 전문가'입니다.
                                아래 제공된 한 학생의 '학기 중 누적된 화학 탐구 보고서 평가 조각'들을 종합하여, 학교생활기록부 '세부능력 및 특기사항(세특)' 난에 즉시 복사해 넣을 수 있는 하나의 매끄럽고 완벽한 문단을 작성하십시오.

                                [Synthesis Principles - 작성 원칙]
                                1. 사실 기반(No Hallucination): 반드시 제공된 평가 기록 내의 구체적인 실험 내용과 데이터만을 활용하십시오. 기록에 없는 실험이나 성과를 임의로 지어내면 절대 안 됩니다.
                                2. 유기적 연결과 심화(Growth Story): 여러 조각의 기록을 단순 나열하지 마십시오. 학생이 첫 실험에서 가졌던 호기심이나 부족했던 점이, 다음 실험을 통해 어떻게 보완되고 깊어졌는지(탐구력 -> 문제해결력 -> 적용) 하나의 흐름으로 엮어주십시오.
                                3. 객관적 전문성: "매우 훌륭함", "천재적임" 같은 영혼 없는 감정적 칭찬은 모두 배제하십시오. 대신 "~오차를 분석하고 ~대안을 제시하는 등 뛰어난 문제해결력을 보임"과 같이 '구체적 행위'를 통해 우수성이 자연스럽게 증명되도록 서술하십시오.
                                4. 약점의 강점화(Crucial): 만약 누적 기록 중에 '오개념'이나 '실패한 실험'에 대한 기록이 있다면, 이를 부정적으로 쓰지 마십시오. 오히려 "초기에는 ~한 오개념을 보였으나, 추가 탐구를 통해 ~원리를 깨닫고 스스로 교정함"과 같이 지적 성장과 자기주도적 학습 능력을 돋보이게 하는 장치로 활용하십시오.

                                [Output Format - 출력 형식]
                                - 길이: 공백 포함 500자 내외로 작성할 것 (나이스 시스템 입력 최적화).
                                - 종결어미: 모든 문장은 반드시 '~함.', '~보임.', '~분석함.', '~탐구함.' 등의 명사형 종결어미로 깔끔하게 끝낼 것.
                                - 형태: 줄바꿈 없이 하나의 짜임새 있는 문단으로 출력할 것.
